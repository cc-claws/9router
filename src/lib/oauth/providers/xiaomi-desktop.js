...
import { XIAOMI_DESKTOP_CONFIG } from "../constants/oauth.js";

// ───────────────────────────────────────────────────────────────────────────
// Xiaomi MiMo Desktop OAuth helpers
...
export function decryptCallback(privateKeyDer, encryptedB64) {
  ...
  return {
    uid: parsed.uid || null,
    sk: parsed.sk || null,
    url: parsed.url || XIAOMI_DESKTOP_CONFIG.defaultBaseUrl,
  };
}
...
export function buildAuthorizeUrl(publicKey, redirectUri, keyName) {
  const params = new URLSearchParams({
    ...
    kn: XIAOMI_DESKTOP_CONFIG.kn,
  });
  if (keyName) params.set("key_name", keyName);
  return `${XIAOMI_DESKTOP_CONFIG.platformUrl}/authorize?${params.toString()}`;
}
