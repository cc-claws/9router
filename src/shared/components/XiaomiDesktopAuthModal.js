...
/**
 * Xiaomi MiMo Desktop Auth Modal
 *
 * Auto-imports credentials from the local MiMo Desktop auth.json (~/.local/share/mimocode/auth.json).
 * If auto-import fails, offers a one-click browser OAuth fallback.
 */
export default function XiaomiDesktopAuthModal({ isOpen, onSuccess, onClose }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const res = await fetch("/api/oauth/xiaomi-desktop/auto-import");
        ...
        if (data.found && data.apiKey) {
          ...
          setPhase("not-found");
          setError(data.error || "MiMo Desktop credentials not found on this machine.");
        }
      } catch {
        if (!cancelled) {
          setPhase("not-found");
          setError("Failed to read local MiMo Desktop credentials.");
        }
      }
    })();
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await fetch("/api/oauth/xiaomi-desktop/api-key", {
        method: "POST",
  ...
  const handleStartOAuth = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/oauth/xiaomi-desktop/authorize?state=${state}`);
      const data = await res.json();
  ...
  const handlePollOAuth = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/oauth/xiaomi-desktop/poll-status?state=${oauthState}`);
      const data = await res.json();

      if (data.status === "done" && data.result) {
        // Exchange to create the connection
        const exRes = await fetch("/api/oauth/xiaomi-desktop/exchange", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <Modal isOpen={isOpen} title="Connect Xiaomi MiMo Desktop" onClose={onClose}>
            ...
            <div className="bg-green-50 dark:bg-green-900/20 p-3 rounded-lg border border-green-200 dark:border-green-800">
              <div className="flex gap-2">
                ...
                <div className="text-sm text-green-800 dark:text-green-200">
                  <p className="font-medium">MiMo Desktop credentials found!</p>
              ...
              <div className="flex gap-2">
                <Button
                  onClick={() => {
                    setPhase("detecting");
                    // Re-trigger detect
                    fetch("/api/oauth/xiaomi-desktop/auto-import")
                      .then((r) => r.json())
                      .then((data) => {
                        if (data.found && data.apiKey) {
                          setDetectResult(data);
...
XiaomiDesktopAuthModal.propTypes = {
