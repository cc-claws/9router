...
import { Card, Button, Badge, Input, Modal, CardSkeleton, OAuthModal, KiroOAuthWrapper, CursorAuthModal, XiaomiDesktopAuthModal, IFlowCookieModal, GitLabAuthModal, Toggle, Select, EditConnectionModal, NoAuthProxyCard, ConfirmModal } from "@/shared/components";
...
export default function ProviderDetailPage() {
  ...
  const [showXiaomiDesktopModal, setShowXiaomiDesktopModal] = useState(false);
  ...
  const triggerOAuthConnection = () => {
    ...
    if (providerId === "xiaomi-desktop") {
      setShowXiaomiDesktopModal(true);
      return;
    }
    if (isOAuth) {
      openOAuthConnection();
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:gap-8 sm:px-0">
      ...
      <XiaomiDesktopAuthModal
        isOpen={showXiaomiDesktopModal}
        onSuccess={handleOAuthSuccess}
        onClose={() => setShowXiaomiDesktopModal(false)}
      />
      {providerId === "iflow" && (
        <IFlowCookieModal
