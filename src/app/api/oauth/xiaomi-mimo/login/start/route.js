...
export async function POST(request) {
  try {
    ...
    let proxiedPath = rewriteMimoBases(pageUrl.pathname + pageUrl.search, "toProxy", origin);
    // Fallback: carry the session in the URL for the FIRST navigation — covers
    // browsers that drop the cookie on a window.open() navigation. The proxied
    // response immediately (re)establishes the cookie for subsequent XHRs.
    proxiedPath += `${pageUrl.search ? "&" : "?"}__9r_sess=${encodeURIComponent(encodeSessionCookie(sess))}`;
