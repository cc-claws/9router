...
- **An upstream's `/models` endpoint does NOT necessarily return the account's usable set.** Most return their whole routing catalog, so surfacing it verbatim (or bulk-importing it as custom models) fills the dashboard with entries that answer 403/404/500. Each live resolver filters by its own signal: GitHub Copilot drops `capabilities.type !== "chat"` and `policy.state !== "enabled"` (`open-sse/services/copilotModels.js`); Cursor asks the account-scoped `GetUsableModels` RPC instead (`cursorModels.js`); Cline exposes no such field, so candidates must be *probed* with a real completion (`probeClineModels` in `clinepassModels.js`). When adding a live catalog for a new provider, find its gating signal first — and never treat HTTP 200 as proof a model works (Cline returns `200 + {"error":"empty response content"}` for dead entries; verify `choices[0].message` exists).
